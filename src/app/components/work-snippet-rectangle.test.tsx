...
import WorkSnippetRectangle from './work-snippet-rectangle';
...
describe(WorkSnippetRectangle.name, () => {

  it("uses correct link attributes", () => {
    const { getByTitle } = render(<WorkSnippetRectangle work={completeWork} />);
  ...
  it("uses correct src attribute when cover_id is provided", () => {
    const { getByAltText } = render(<WorkSnippetRectangle work={completeWork} />);
  ...
  it("uses fallback when cover_id is not provided", () => {
    const { getByAltText } = render(<WorkSnippetRectangle work={incompleteWork} />);
    const cover = getByAltText("Work cover");
    expect(cover.getAttribute("src")).toContain("fallback-cover.jpg");
  });

  it("displays rating correctly when provided", () => {
    const { queryByText, getByTestId } = render(<WorkSnippetRectangle work={completeWork} />);
  ...
  it("displays rating correctly when not provided", () => {
    const { queryByText, getByTestId } = render(<WorkSnippetRectangle work={incompleteWork} />);
