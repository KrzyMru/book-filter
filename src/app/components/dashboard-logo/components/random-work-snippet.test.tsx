...
import RandomWorkSnippet from './random-work-snippet';
...
describe(RandomWorkSnippet.name, () => {

  it("uses correct link attributes", () => {
    const { getByTitle } = render(<RandomWorkSnippet work={completeWork} />);
  ...
  it("uses correct src attribute when cover_id is provided", () => {
    const { getByAltText } = render(<RandomWorkSnippet work={completeWork} />);
  ...
  it("uses fallback when cover_id is not provided", () => {
    const { getByAltText } = render(<RandomWorkSnippet work={incompleteWork} />);
    const cover = getByAltText("Work cover");
    expect(cover.getAttribute("src")).toContain("fallback-cover.jpg");
  });

  it("displays rating correctly when provided", () => {
    const { queryByText, getByTestId } = render(<RandomWorkSnippet work={completeWork} />);
  ...
  it("displays rating correctly when not provided", () => {
    const { queryByText, getByTestId } = render(<RandomWorkSnippet work={incompleteWork} />);
